...
class Animal(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode, num_retries=10):
        """
        Construct the Animal images loader with a given csv file. The format of
        the csv file is:
        ```
        original_video_id video_id frame_id path labels type 
        ```
        Args:
            cfg (CfgNode): configs.
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for Animal".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        logger.info("Constructing Animals {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        with g_pathmgr.open(path_to_file, "r") as f:
            ...
            for clip_idx, path_label in enumerate(f.read().splitlines()):
                ...
                _, video_id, frame_id, path, labels, _ = path_label.split(
                ...
                if video_id == last_video_id:
                    ...
                    self._labels.append([int(label)] for label in labels)
                    self._spatial_temporal_idx.append(video_id)
                    #self._video_meta[clip_idx * self._num_clips + idx] = {}
        assert (
            len(self._path_to_frames) > 0
        ), "Failed to load Animal split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing Animal dataloader (size: {}) from {}".format(
                len(self._path_to_frames), path_to_file
            )
        )
