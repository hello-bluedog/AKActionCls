...
_MODELS = {
    ...
    "ViT-B/16": os.path.join(MODEL_PATH, "vit_b16_k400_bias.pth"),
...
class Extractor(nn.Module):
    ...
    def attention(self, x, y):
        ...
        q = q.view(Tx, N, self.attn.num_heads, self.attn.head_dim).permute(1, 2, 0, 3).contiguous()
        k = k.view(Ty, N, self.attn.num_heads, self.attn.head_dim).permute(1, 2, 0, 3).contiguous()
        v = v.view(Ty, N, self.attn.num_heads, self.attn.head_dim).permute(1, 2, 0, 3).contiguous()
        aff = (q @ k.transpose(-2, -1).contiguous() / (self.attn.head_dim ** 0.5))

        aff = aff.softmax(dim=-1)
        out = aff @ v
        out = out.permute(2, 0, 1, 3).contiguous().flatten(2)
    ...
    def forward(self, x):
        ...
        for i, resblock in enumerate(self.resblocks):
            ...
            if i in self.return_list:
                ...
                tmp_feats = tmp_feats.permute(1, 3, 2, 0).contiguous().reshape(N, C, T_down, H, W)
                tmp_feats = self.dpe[j](tmp_feats.clone()).view(N, C, T_down, L - 1).permute(3, 0, 2, 1).contiguous()
                tmp_x[1:] = tmp_x[1:] + tmp_feats
                # global block
                tmp_x = tmp_x.permute(2, 0, 1, 3).contiguous().flatten(0, 1)  # T * L, N, C
